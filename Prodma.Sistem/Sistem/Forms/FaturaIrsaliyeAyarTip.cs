...
{
    public partial class FaturaIrsaliyeAyarTip : TanitimSablon
    {
        #region degiskenler
        ...
        public override void Doldur()
        {
            ...
            faturaIrsaliyeAyarTipVm.FATURA_IRSALIYE_TIP_ID = (int)FaturaIrsaliyeAyarTipEn.FATURA; 
        ...
        {
            FaturaIrsaliyeAyarTipVm k = (FaturaIrsaliyeAyarTipVm)ModelVm;
            k.FATURA_IRSALIYE_TIP_ID = (int)FaturaIrsaliyeAyarTipEn.FATURA; 
            kulcntrl.Ekle(k);
            lblBilgi.Text = kulcntrl.MesajYaz();
            islemTamamDegil = kulcntrl.islemTamamDegilCntrl;  
        }
        ...
        public override void ProcessGridKey(GridView view, KeyEventArgs args, int FocusedRowHandle, DevExpress.XtraGrid.Columns.GridColumn FocusedColumn)
        {
            if (Control.ModifierKeys == Keys.Control)
            {
                if (args.KeyCode == Keys.F9)
                {
                    ...
                    //FaturaIrsaliyeAyarTipVm k = (FaturaIrsaliyeAyarTipVm)ModelVm;
                    //frm.ayarId = k.ID;
                    //frm.ayarTipId = (int)FaturaIrsaliyeAyarTipEn.FATURA;
                    //frm.ShowDialog();
        ...
        private void TabIndex2_Ac(int index)
        {
            ...
            if (k.FATURA_IRSALIYE_TIP_ID == 1)//İRSALİYE
            {
                if (usrIrsaliyeYaziciAyar == null)
                {
                    usrIrsaliyeYaziciAyar = new usrIrsaliyeYaziciAyar();
                }
                usrIrsaliyeYaziciAyar.FtuIrsAyarVm.FATURA_IRSALIYE_AYAR_TIP_ID = k.ID;
                this.tabPage2.Controls.Clear();
                this.tabPage2.Controls.Add(this.usrIrsaliyeYaziciAyar);
            }
            if (k.FATURA_IRSALIYE_TIP_ID == 2)//FATURA
            {
                if (usrFaturaAyar == null)
                {
                    usrFaturaAyar = new usrFaturaAyar();
                }
                //usrFaturaAyar.FtuIrsAyarVm.FATURA_IRSALIYE_AYAR_TIP_ID = k.ID;
                usrFaturaAyar.faturaIrsaliyeAyarID = k.ID;
                usrFaturaAyar.Doldur();
                this.tabPage2.Controls.Clear();
                this.tabPage2.Controls.Add(this.usrFaturaAyar);
            }
        }
        #endregion
