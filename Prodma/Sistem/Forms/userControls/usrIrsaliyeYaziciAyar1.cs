...
namespace Prodma.Sistem.Forms
{
    public partial class usrIrsaliyeYaziciAyar : usrControlSablon
    {
        ...
        int faturaIrsaliyeAyarID;
        bool control;
        public usrIrsaliyeYaziciAyar()
        {
            InitializeComponent();
            FtuIrsAyarVm = FtuIrsCntrl.Liste_Al(null)[0];
            this.Controls.Add(navBar);
            Control_Ayarla_Kayit(this.Controls, ListDenemeService.GetALANLAR("FATURA_IRSALIYE_AYAR", 1), null, null, false);

            if (FtuIrsAyarVm != null)
            {
                ...
                foreach (Control control in this.Controls)
                {
                    if (control.GetType() == typeof(DevExpress.XtraEditors.TextEdit))
                    {
                        ...
                        if (fia.Name + "txt" == ctrl.Name)
                        {
                            ...
                            if (propertyValue != null && Convert.ToDecimal(propertyValue)>0)
                            {
                                ctrl.Enabled = true;
