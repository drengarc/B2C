...
namespace Prodma.Parent
{
    public partial class MDIParent : DevExpress.XtraEditors.XtraForm
    {
        ...
        public void MenuleriDoldur()
        {
                ...
                {
                    subParent = false;
                    if (menu.M_MENU_ID == 0 && (menu.KULLANICI_GORMESIN == null || menu.KULLANICI_GORMESIN == false))
                    {
                    ...
                    else
                    {
                        for (int j = 0; j < k; j++)
                        {
                            if (menu.M_MENU_ID == ustIdler[j] && (menu.KULLANICI_GORMESIN == null || menu.KULLANICI_GORMESIN == false))
                            {
                                subParentMenus.Add(menu);
                                subParent = true;
                            }
                        }
                        if (subParent == false && (menu.KULLANICI_GORMESIN == null || menu.KULLANICI_GORMESIN == false))
                        {
                            menus.Add(menu);
                        }
                    }
        ...
        private void DropDownItems_Click(object sender, EventArgs e)
        {
            try
            {
                YardimciIslemler.ProgramBeklemeGoster();
            ...
            {
                YardimciIslemler.ProgramBeklemeDurdur();
            }
            YardimciIslemler.ProgramBeklemeDurdur();
        }
        public void  Prodma_Form_Ac(Form frm,int menuId)
        {
            Sablon objfrmSChild = (Sablon)frm;
